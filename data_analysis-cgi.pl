...
sub get_xls_filename {
  # takes uploaded CSV filename as argument
  my $uploaded_csv = shift;
  my $download_xlsx = $uploaded_csv;

  $download_xlsx =~ s/\.txt$/.xlsx/;
  $download_xlsx =~ s/.*(\\|\/)//g;
...
sub get_sample_names {
  #sample names should be provided in order used as a comma-seperated list
  return split ( ',', shift );
}

sub get_solvent_names {
  #solvent names should be provided in order used as a comma-seperated list
  return split ( ',', shift );
}
...
my $cgi = CGI->new;
my $title = 'CSV Data to XLSX';
my $password = 'cellwall';

if ( defined ( $cgi->upload( 'upload' ) ) )
{
  if ( $cgi->param( 'password' ) ne $password )
  {
    # abort upload processing
    print $cgi->header,
      $cgi->start_html( $title ),
      $cgi->h1( 'Acess Denied' ),
      $cgi->end_html;
  }
  elsif ( !$cgi->param( 'samples' ) || !$cgi->param( 'solvents' ) )
  {
    print $cgi->header,
      $cgi->start_html( $title ),
      $cgi->h1( 'Parameters not provided.' ),
      $cgi->end_html;
  }
  else
  {
    # process uploaded CSV file
    my $csv_fh = $cgi->upload( 'upload' )->handle;
    my $filename_csv = $cgi->param( 'upload' );

    my $filename_xls = get_xls_filename ( $filename_csv );
    my @solvents = get_solvent_names ( $cgi->param( 'solvents' ) );
    my @samples = get_sample_names ( $cgi->param( 'samples' ) );
    ...
    process_csv_file ( \%csv, $csv_fh );
    add_data_fields ( $csv{'data'}, \@samples, \@solvents );

    print $cgi->header( -type=> 'application/x-xlsx',
                        -attachment=> "$filename_xls" );
...
{
  # display upload page
  my $textfield_size = 50;

  print $cgi->header,
        $cgi->start_html( $title ),
        $cgi->h1( $title );

  print $cgi->start_multipart_form( { -style=> 'text-align:center' } ),
        '<p><label>CSV file: ',
        $cgi->filefield( -name=> 'upload', -size=> $textfield_size ),
        '</label></p>',
        '<p><label>Samples (comma seperated): ',
        $cgi->textfield( -name=> 'samples', -size=> $textfield_size ),
        '</label></p>',
        '<p><label>Solvents (comma seperated): ',
        $cgi->textfield( -name=> 'solvents', -size=> $textfield_size ),
        '</label></p>',
        '<p><label>Password:' . $cgi->password_field( -name=> 'password',
                                      -size=> 20,
                                      -maxlength=> 16 ) . '</label></p>',
        '<p>' . $cgi->submit( -value=> 'Upload' ) . '</p>',
        $cgi->end_form;
